...
# Импорт psutil для работы с процессами
try:
    import psutil
except ImportError:
    psutil = None
...
# === Служебные функции для управления процессом бота ===

def _get_pid_file_path() -> Path:
    # Эта функция теперь зависит от PID_FILENAME из bot_entrypoint
    try:
        ...
        # Fallback, если настройки не загрузились
        return Path.cwd() / "project_data" / "sdb_bot.pid"

def _read_pid_from_file(pid_file: Path) -> Optional[int]:
    """Читает PID из файла с расширенной обработкой ошибок."""
    ...
    try:
        ...
        if not pid_str:
            sdb_console.print(f"[yellow]Предупреждение: PID-файл {pid_file} пуст[/yellow]")
            return None
            
        if not pid_str.isdigit():
            sdb_console.print(f"[yellow]Предупреждение: PID-файл {pid_file} содержит некорректное значение: '{pid_str}'[/yellow]")
            return None
            
        pid = int(pid_str)
        if pid <= 0:
            sdb_console.print(f"[yellow]Предупреждение: Некорректный PID в файле {pid_file}: {pid}[/yellow]")
            return None
            
        return pid
    except (IOError, OSError) as e:
        sdb_console.print(f"[red]Ошибка доступа к PID-файлу {pid_file}: {e}[/red]")
        return None
    except ValueError as e:
        sdb_console.print(f"[yellow]Ошибка парсинга PID из файла {pid_file}: {e}[/yellow]")
        return None
    except Exception as e:
        sdb_console.print(f"[red]Неожиданная ошибка при чтении PID из файла {pid_file}: {e}[/red]")
        return None
...
try:
    ...
    from cli_commands.control_cmd import control_app
    from cli_commands.bot_cmd import bot_app 
    from cli_commands.logs_cmd import logs_app
    from cli_commands.license_cmd import license_app # <-- ИЗМЕНЕНИЕ: импортируем новую команду
    from core.bot_entrypoint import run_sdb_bot, PID_FILENAME 
except ImportError as e:
...
cli_main_app.add_typer(logs_app, name="logs", help="📋 Просмотр и управление логами SDB.")
cli_main_app.add_typer(license_app, name="license", help="🔑 Управление лицензией SwiftDevBot.") # <-- ИЗМЕНЕНИЕ: регистрируем новую команду
# control_app оставляем для обратной совместимости
cli_main_app.add_typer(control_app, name="control", help="🚦 Управление процессом бота (для совместимости, используйте прямые команды).") 
cli_main_app.add_typer(bot_app, name="bot", help="🤖 Управление Telegram-ботом SDB (команды, статус).") 

# УДАЛЯЕМ lite_cmd
# from cli_commands.lite_cmd import add_lite_commands
# add_lite_commands(cli_main_app)

# (весь остальной код файла sdb.py остается БЕЗ ИЗМЕНЕНИЙ)
# ...
...
def start_bot_command(
    ...
    if pid_file_path.exists():
        try:
            ...
            if sys.platform != "win32":
                os.kill(pid, 0) 
                sdb_console.print(f"[yellow]SDB бот уже запущен (PID: {pid}). Используйте './sdb.py stop' для остановки.[/yellow]")
                ...
                if pid_file_path.exists():
                    try:
                        ...
                        if actual_pid_from_file_str.isdigit():
                            ...
                            sdb_console.print("Для просмотра статуса используйте: [cyan]./sdb.py status[/cyan]")
                            sdb_console.print("Для остановки используйте: [cyan]./sdb.py stop[/cyan]")
...
@cli_main_app.command(name="stop", help="⏹️ Остановить SDB бота.")
    ...
    try:
        ...
        if _is_process_running(pid):
            if force:
                ...
                sdb_console.print(f"  Попробуйте './sdb.py stop --force' или остановите процесс вручную (kill {pid}).")
                raise typer.Exit(code=1) 
        else: 
...
@cli_main_app.command(name="status", help="📊 Показать текущий статус SDB бота.")
def status_bot_command():
    ...
    if pid and _is_process_running(pid):
        sdb_console.print(f"[green]● SDB бот запущен (PID: {pid})[/green]")
        ...
        if psutil:
            try:
                ...
                with p.oneshot(): 
                    ...
                    sdb_console.print(f"[cyan]Время работы:[/] {uptime_delta}")
                    sdb_console.print(f"[cyan]Память (RSS):[/] {memory_mb:.1f} МБ")
                    sdb_console.print(f"[cyan]Создан:[/] {create_time.strftime('%Y-%m-%d %H:%M:%S')}")
            except Exception as e:
                sdb_console.print(f"[dim]Ошибка получения доп. информации: {e}[/dim]")
    else:
        sdb_console.print("[red]● SDB бот остановлен[/red]")
        if pid:
            sdb_console.print(f"[dim](PID {pid} из файла не активен)[/dim]")
    
    sdb_console.print(f"\n[cyan]Python:[/] {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}")
    sdb_console.print(f"[cyan]Платформа:[/] {sys.platform}")
    sdb_console.print(f"[cyan]SDB корень:[/] {project_root}")

@cli_main_app.command(name="restart", help="🔄 Перезапустить SDB бота.")
def restart_bot_command(
    ...
    if not sdb_executable_str:
        sdb_console.print(f"[bold red]Не удалось определить исполняемый файл SDB CLI. Проверьте sys.argv[0]: {sys.argv[0]}[/bold red]")
        raise typer.Exit(1)
    ...
    try:
        ...
        if stop_process_result.returncode != 0:
            ...
            if not typer.confirm("Продолжить попытку запуска, несмотря на ошибку остановки?", default=False):
                sdb_console.print("[bold yellow]Перезапуск отменен пользователем.[/bold yellow]")
                raise typer.Exit(code=1)
            sdb_console.print("[yellow]Продолжение перезапуска, несмотря на ошибку остановки...[/yellow]")
            stop_failed = True
        else:
            sdb_console.print("[green]Фаза остановки успешно завершена (бот остановлен или не был запущен).[/green]")

    except Exception as e_stop_wrapper:
        sdb_console.print(f"[bold red]Непредвиденная ошибка при вызове подпроцесса 'stop': {e_stop_wrapper}[/bold red]")
        if not typer.confirm("Продолжить попытку запуска, несмотря на ошибку?", default=False):
            sdb_console.print("[bold yellow]Перезапуск отменен пользователем.[/bold yellow]")
            raise typer.Exit(code=1)
        sdb_console.print("[yellow]Продолжение перезапуска...[/yellow]")
        stop_failed = True

    if not stop_failed:
        sdb_console.print("[dim]Ожидание 1 секунда перед запуском...[/dim]")
        time.sleep(1)
    ...
    try:
        ...
        if background:
            ...
            if start_process_result.stdout:
                 sdb_console.print(f"[dim cyan]Вывод 'start':[/]\n{start_process_result.stdout.strip()}")
            if start_process_result.stderr: 
                 sdb_console.print(f"[dim yellow]Вывод (возможно, ошибки) от 'start':[/]\n{start_process_result.stderr.strip()}")
            if start_process_result.returncode != 0:
                sdb_console.print(f"[bold red]Ошибка на фазе запуска бота (код: {start_process_result.returncode}).[/bold red]")
                raise typer.Exit(code=1)
            sdb_console.print("[green]Команда запуска бота успешно вызвана (интерактивный режим).[/green]")

    except Exception as e_start_wrapper:
        sdb_console.print(f"[bold red]Непредвиденная ошибка на этапе запуска: {e_start_wrapper}[/bold red]")
        raise typer.Exit(code=1)

    sdb_console.print("\n[bold green]Команда перезапуска завершена.[/bold green]")
    if background:
        sdb_console.print("  Проверьте статус бота через: [cyan]./sdb.py status[/cyan]")
